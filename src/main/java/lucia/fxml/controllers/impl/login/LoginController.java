...
import main.java.lucia.client.protocol.packet.outgoing.order.PacketOutGetOrderNumberOfDay;
 ...
 */
public class LoginController implements Controller, PacketHandler {
    ...
    @PacketEventHandler
    public void onDayGet(PacketInFoundOrderNumberOfDay packet){
        if(packet.getDay().isEqual(LocalDate.now().minusDays(1))){
            setYesterdaysOrders(packet.getOrders());
        }
    }
...
//    }, 15);
    ...
    public void openPreview() {
        Platform.runLater(() -> {
            updatePreviewPane();
            PacketSender.getCurrentPacketSender().sendMessage(new PacketOutGetOrderNumberOfDay(LocalDate.now().minusDays(1)));
            previewPane.toFront();
