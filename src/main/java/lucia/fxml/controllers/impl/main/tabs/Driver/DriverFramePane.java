...
public class DriverFramePane implements Controller {
  ...
  public void loadDeliveries() {
    ...
    if(anyDeliveries)
      for (int x = 0; x < OrderManager.INSTANCE.getPendingOrdersDelivery().size(); x++)
        if(OrderManager.INSTANCE.getPendingOrdersDelivery().get(x).isDelivery()) {
          Label label = new Label(OrderManager.INSTANCE.getPendingOrdersDelivery().get(x).getCustomerDetails().getAddress().toString());
