...
//            .getController(ControllerType.ENTER_NUMBER_PANE_CONTROLLER);
  ...
  public void setCount() {
    for (int x = 0; x < orderManager.getPendingOrders().size(); x++) {
      Label label = new Label(" " +String.valueOf(orderManager.getPendingOrders().get(x).getOrderNumber()));
  ...
  public void setName() {
    for (int x = 0; x < orderManager.getPendingOrders().size(); x++) {
      CustomerDetails c = orderManager.getPendingOrders().get(x).getCustomerDetails();
  ...
  public void setTime() {
    for(int x = 0; x < orderManager.getPendingOrders().size(); x++) {
      Label time = new Label(" " +orderManager.getPendingOrders().get(x).getFormattedSetTime());
  ...
  public void setSince(){
    for(int x = 0; x < orderManager.getPendingOrders().size(); x++) {
      Label since = new Label(" " + orderManager.getPendingOrders().get(x).timeSince());
  ...
  public void setTotal() {
    for (int x = 0; x < orderManager.getPendingOrders().size(); x++) {
      long total = orderManager.getPendingOrders().get(x).getGrandTotalLongWithTax();
  ...
  public Order getHighlightedOrder(){
    return orderManager.getPendingOrders().get(gridHighlighter.getHighLightedRow());
  }
