...
/**
 * Finds a location on the map given an address.
...
function findLocation(input) {
  geocoder.geocode({
    ...
    if (type === 'GEOMETRIC_CENTER') {
      ...
      removeAllMarkers();
      response.warningMessage = 'Address Does Not Exist';
    ...
    if (status === 'OK') {
      ...
      map.setZoom(14);
      if(status) {
          if (google.maps.geometry.poly.containsLocation(latLng, stores[0].poly)) {
            response.warningMessage = 'Address is within Henderson`s Area.';
            javaBridge.geocodeCallBack(response);
            javaBridge.testInfo(results[0].address_components[2].long_name + ' | ' + results[0].address_components[2].short_name);
            return;
            infowindowContent.children['place-address'].textContent = address;
            response.json = JSON.stringify(results);
            response.type = stores[0].type;
            response.name = stores[0].name;
            finishMarker(marker, stores[0].type);
          }
        for (const store of stores) {
          if(google.maps.geometry.poly.containsLocation(latLng, store.poly)) {
            response.warningMessage = 'Address is within ' + store.name + '`s Area.';
            javaBridge.geocodeCallBack(response);
            javaBridge.testInfo(results[0].address_components[2].long_name + ' | ' + results[0].address_components[2].short_name);
            return;
            infowindowContent.children['place-address'].textContent = address;
        ...
        response.warningMessage = 'Address is NOT within an Area.';
        javaBridge.geocodeCallBack(response);
        javaBridge.testInfo(results[0].address_components[2].long_name + ' | ' + results[0].address_components[2].short_name);
        return;
      }
...
function SwitchMapController(controlDiv) {
  ...
  goWinnipegUI.title = 'Click to go to the Winnipeg Map';
  ...
  goSaskatoonUI.title = 'Click to go to the Saskatoon Map';
